...
switch rowSamp   
    ...
    case '1/3'      
        for i = 1:npt
            sampChosen = randi([0 2]);
            if ~isequal(AMatrix(sampChosen*npt+i,:),zeros(1,nbasis))       %To account for margins of domain
                AMatrix((mod(sampChosen+1,3))*npt+i,:) = 0;
                AMatrix((mod(sampChosen-1,3))*npt+i,:) = 0;
            end 
        end
