...
for k = 1:niter
    k 
    xmod = x + npstep(k,1)*xnoise(stepx1/2,n)    %adding noise to avoid local minima
...
for k = 1:niter
    k 
    xmod = x + npstep(k,2)*xnoise(stepx1/2,n);   %adding noise to avoid local minima
    xmod = boundx(dom(1,1),dom(1,2),xmod); %domain bounded
    x1 = xmod(:,1);
    x2 = xmod(:,2);
    F = double(subs(f));
...
for k = 1:niter
    k 
    xmod = x + npstep(k,3)*xnoise(stepx1/2,n);   %adding noise to avoid local minima
    xmod = boundx(dom(1,1),dom(1,2),xmod); %domain bounded
    x1 = xmod(:,1);
    x2 = xmod(:,2);
    F = double(subs(f));
